...
function addGlobalTokens(tokens){
    ...
    //console.log("globalTokens: ", globalTokens)
}
...
function getPredTrueRelevantFieldsTableHtml(predTrueToken, relevantFields){
    ...
    return "<table>"+relevantFields.filter(rf=>rf!=GLOBAL_TOKENS_INDEX_PROP).map(rf => "<tr>"+
            "<td><strong>"+rf+":</strong></td>"+
            "<td class='relevant-field-true'>"+predTrueToken.true[rf]+"</td>"+
            "<td class='relevant-field-pred'>(pred: "+predTrueToken.pred[rf]+")</td>"+"</tr>"
    ).join("\n")+
    "</table>"+
    "<span class='nlp-global-tokens-index'>globalTokens index: "+
    predTrueToken[GLOBAL_TOKENS_INDEX_PROP]+
    "</span>"
}
...
function updateTokenPredTrueComparisonSnapshot(snapshotDivId, tokenHtmlTag, relevantFields=[]){
    if( !tokenHtmlTag.classList.contains(TRUE_NEGATIVE_LABEL) ){
        ...
        if( snapShotDiv!== null ){
            ...
            //console.log("updateTokenPredTrueComparisonSnapshot() tokenHtmlTag:", tokenHtmlTag, ", token: ", token)
...
function visualizePredTrueComparison(predTokens, trueTokens, predField, separatorsField="MISC", relevantFields=[]){
    ...
    const contentDiv = visualizeDocument(tokens, relevantFields)
    const styleTag = `<style>${CSS_AS_STRING}</style>`

    // adding the event listeners
    setTimeout(()=>{
        const selector = `#${vizDivId} .${NLP_LABELLED_TOKEN_CLASS}`
        const labelledTokens = document.querySelectorAll(selector)
        labelledTokens.forEach(tokenTag => tokenTag.addEventListener("mouseover",e=>{
            updateTokenPredTrueComparisonSnapshot(snapShotDivId, e.currentTarget, relevantFields)
